...
export const ListingOptionSchema = z.object({
  ...
  petPolicy: z.string().min(1).optional(),
  contractRisk: z.string().min(1).optional(),
  moveInDate: z.string().min(1),
  roommateFit: z.string().min(1).optional(),
  /** Lightweight card risk cue (recommended new field). */
  keyRisk: z.string().min(1).optional(),

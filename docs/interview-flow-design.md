...
当前 AI-MBTI v6.0 流程分为：背景访谈 -> 两部分问卷 -> 一轮中途校准 -> 报告生成 -> 报告反馈。运行时主链路是单一 researcher agent，不再使用旧版 Agent A / Agent B 双代理编排。

旧文档中的“16-20 道单次问卷”“Agent A 必须同时问职业和 AI 使用方式”“Agent B 每轮给 A directive”属于 legacy 设计。代码里仍有兼容入口，但新产品口径以 Phase 6 两部分 24 题为准。
...
- 第一部分四个维度各 2 题，每维 1 正向 + 1 反向。
- 第二部分四个维度各 4 题，每维 2 正向 + 2 反向。
- 每部分必须有 6 道 `scenario="习惯"` 的习惯题和 6 道具体或半具体场景题。
- 单部分每个维度必须同时有正向题和反向题，允许 2/1 分布。
- 两部分合计后，每个维度必须 6 题，且 3 道正向 + 3 道反向。
- 场景题不能使用泛场景，例如“日常使用 AI”。
...
- 第二部分：16 道 `hybrid_batch2` 题，8 习惯 + 8 场景，两部分合计每维 3 正向 + 3 反向。

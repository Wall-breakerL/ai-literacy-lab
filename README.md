...
> 通过轻量访谈、三批定制问卷和报告反馈，测量你与 AI 协作的习惯与风格。

## 是什么

一个基于“访谈 + 三批定制问卷 + 报告 + 反馈”的 AI-MBTI 测评系统。用户先与单一 researcher agent 进行轻量背景访谈，系统收集职业/身份、AI 使用背景与当前目标；随后进入 Phase 6 的 24 题问卷流程：三批 8 题，中间穿插两段校准对话。用户作答后，服务端确定性计算四维得分，LLM 只负责生成个性化解释、建议和 prompt 模板。
...
- Phase 6 问卷固定为 24 题：`habit_batch`、`scenario_batch`、`mixed_batch` 各 8 题。
- 第一批和第二批后各有一段 mid-dialogue，用于确认题目感受、调整真实场景和场景颗粒度。
...
该脚本会调用中途对话开场、三批问卷生成、报告 `batchAnswers` 入口和反馈保存接口。默认不调用反馈 LLM 对话；需要真实 LLM smoke 时设置 `RUN_LLM_SMOKE=1`。如果 Notion 已配置，反馈 smoke 会写入真实 Notion；否则写入 `.local-debug/feedback/`。
...
```
/            首页
/interview      AI-MBTI 访谈 + 三批问卷 + 中途校准页
